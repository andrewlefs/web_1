...
class Tank_auth
{
	...
	function make_new_pass($new_pass)
	{
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'status' => '', 'credit' => '0'));
	...
	{
		return $this->ci->users->get_user_by_id($user_id,'1');
	...
	function create_user($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			if ($email_activation) {
				$data['new_email_key'] = rand(1000, 9999);
			}
